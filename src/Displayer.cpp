...
/* Constructors */
Displayer::Displayer(Map &map, Player &p, int width, int height)
    : map(map), player(p), m_width(width), m_height(height),
      m_updateReady(false), m_isOpen(true) {}
...
void Displayer::waitReady() { m_ready.lock(); }

void Displayer::run() {
  // Open window
  m_pWindow = std::make_unique<sf::RenderWindow>(
      sf::VideoMode(m_width, m_height), "TP3 IA, the endless forest.");
...
void Displayer::drawScene() {
  ...
  // Scale size
  unsigned int sizex = this->m_pWindow->getSize().x / this->map.size();
  float scalingx = (float)sizex / 100.;
  unsigned int sizey = this->m_pWindow->getSize().y / this->map.size();
  float scalingy = (float)sizey / 100.;
  ...
  for (size_t r = 0; r < this->map.size(); ++r) {
    for (size_t c = 0; c < this->map.size(); ++c) {
      ...
      if (caseToRender.portal) {
        ...
        sprite.setTexture(
            ((r + c) % 2 == 0) ? textureWalkable1 : textureWalkable2, true);
      }

      sprite.setPosition(sizex * c, sizey * r);
      sprite.setScale(scalingx, scalingy);
      m_pWindow->draw(sprite);

      if (this->map.hasPoop(c, r)) {
        sf::Sprite spritePoop;
        spritePoop.setPosition(sizex * c, sizey * r);
        spritePoop.setTexture(texturePoop, true);
        spritePoop.setScale(scalingx, scalingy);
        m_pWindow->draw(spritePoop);
      }
      if (this->map.hasWind(c, r)) {
        sf::Sprite spriteWind;
        spriteWind.setPosition(sizex * c, sizey * r);
        spriteWind.setTexture(textureWind, true);
        spriteWind.setScale(scalingx, scalingy);
      ...
      if (c == posPlayer.x && r == posPlayer.y) {
        ...
        spritePlayer.setPosition(sizex * c, sizey * r);
        spritePlayer.setTexture(texturePlayer, true);
        spritePlayer.setScale(scalingx, scalingy);
